const input = [`Game 1: 10 green, 5 blue; 1 red, 9 green, 10 blue; 5 blue, 6 green, 2 red; 7 green, 9 blue, 1 red; 2 red, 10 blue, 10 green; 7 blue, 1 red`,
...
const redRegex = /(\d*)( red)/gm;
const greenRegex = /(\d*)( green)/gm;
const regexes = [[14, blueRegex], [12, redRegex], [13, greenRegex]];

function parseGameResults(gameResults) {
  const id = +[...gameResults.matchAll(/(Game )(\d*)/gm)][0][2];
  const result = regexes.map(([target, regex]) => {
    return [...gameResults.matchAll(regex)]
      .map((v) => v[1])
      .every((v) => +v <= target);
  })
    .every((x) => x);
...
function findMinimumViableGameState(gameResults) {
  const justTheRegexes = regexes.map(([_, xs]) => xs);
  return justTheRegexes.map((regex) => {
    return [...gameResults.matchAll(regex)]
      .map((x) => +x[1])
      .sort((a,b) => a - b)
      .reverse()[0]
...
function run(input) {
  ...
  const result2 = input
    .map(findMinimumViableGameState)
